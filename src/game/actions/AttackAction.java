...
public class AttackAction extends Action {
	 ...
	 * @param direction the direction where the target is relative to the location
	 *                  of actor
	 */
	...
	public String execute(Actor actor, GameMap map) {
		...
		// Drops items and removes the target if the target is not koopa and is no
		// longer conscious, or if mario holds a wrench which will kill koopa
		if ((!target.isConscious() && !target.hasCapability(Status.DORMANT))
				|| (actor.hasCapability(Status.DORMANTKILL)) && target.hasCapability(Status.DORMANT)) {
